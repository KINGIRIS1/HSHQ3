...
import { User as UserType, NotifyFunction, RecordFile } from '../../types';
...
const SoanBienBanTab: React.FC<SoanBienBanTabProps> = ({ currentUser, isActive, notify, initialRecord, onClearInitialRecord }) => {
  // Mode: 'create' or 'list'
  ...
  // Autofill if initialRecord is passed
  useEffect(() => {
      if (initialRecord) {
          setSearchCode(initialRecord.code || '');
          setFormData(prev => ({
              ...prev,
              TEN_CHU: initialRecord.customerName || prev.TEN_CHU,
              DIA_CHI_CHU: initialRecord.customerAddress || prev.DIA_CHI_CHU,
              OWNERS: [{
                  id: Date.now().toString(),
                  title: 'Ông/Bà',
                  name: initialRecord.customerName || '',
                  address: initialRecord.customerAddress || '',
                  hasSpouse: false,
                  spouseTitle: 'Bà',
                  spouseName: '',
                  spouseAddress: ''
              }],
              SO_THUA_MOI: initialRecord.landPlot || prev.SO_THUA_MOI,
              SO_TO_MOI: initialRecord.mapSheet || prev.SO_TO_MOI,
              DIA_CHI_THUA: initialRecord.address || prev.DIA_CHI_THUA,
              PHUONG: initialRecord.ward || prev.PHUONG,
              SO_GCN: initialRecord.issueNumber || prev.SO_GCN,
              SO_VAO_SO: initialRecord.entryNumber || prev.SO_VAO_SO,
              NGAY_CAP: initialRecord.issueDate || prev.NGAY_CAP,
              DT_CU: initialRecord.area ? initialRecord.area.toString() : prev.DT_CU,
              DT_ODT: initialRecord.residentialArea ? initialRecord.residentialArea.toString() : prev.DT_ODT,
              SO_THUA_CU: initialRecord.landPlot || prev.SO_THUA_CU,
              SO_TO_CU: initialRecord.mapSheet || prev.SO_TO_CU,
          }));
          setMode('create'); // Switch to drafting mode
          notify("Đã tự động điền thông tin hồ sơ vào biên bản!", "success");
          onClearInitialRecord?.();
      }
  }, [initialRecord, onClearInitialRecord, notify]);
