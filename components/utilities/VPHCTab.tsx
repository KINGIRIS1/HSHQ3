...
import { Settings, List, PlusCircle, Save, Search, Loader2, X } from 'lucide-react';
...
interface VPHCTabProps {
    ...
    presetRecord?: RecordFile;
    onClose?: () => void;
}

const VPHCTab: React.FC<VPHCTabProps> = ({ currentUser, notify, presetRecord, onClose }) => {
    ...
    // --- AUTO FILL FROM PRESET RECORD ---
    useEffect(() => {
        if (presetRecord) {
            setFormData(prev => ({
                ...prev,
                NGUOI: presetRecord.customerName || prev.NGUOI,
                NOIO: presetRecord.customerAddress || prev.NOIO,
                CCCD: presetRecord.cccd || prev.CCCD,
                THUA: presetRecord.landPlot || prev.THUA,
                TO: presetRecord.mapSheet || prev.TO,
                DT: presetRecord.area ? presetRecord.area.toString() : prev.DT,
                DC_THUA: presetRecord.address || prev.DC_THUA,
                XA_PHUONG: presetRecord.ward || prev.XA_PHUONG,
                SPH: presetRecord.issueNumber || prev.SPH,
                SVS: presetRecord.entryNumber || prev.SVS,
                NGAYCAPGCN: presetRecord.issueDate || prev.NGAYCAPGCN,
                CHUSDGCN: presetRecord.customerName ? presetRecord.customerName.toUpperCase() : prev.CHUSDGCN
            }));
            setSearchCode(presetRecord.code);
        }
    }, [presetRecord]);

    // --- AUTO SAVE/LOAD CACHE ---
    useEffect(() => {
        if (mode === 'create' && !editingId && !presetRecord) {
    ...
    }, [mode, editingId, presetRecord]);

    useEffect(() => {
        if (!editingId && !presetRecord) {
            localStorage.setItem('CACHE_VPHC_FORM', JSON.stringify(formData));
        }
    }, [formData, editingId, presetRecord]);
    // ----------------------------
