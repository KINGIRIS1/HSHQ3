...
export interface ArchiveRecord {
    ...
    status: 'draft' | 'assigned' | 'executed' | 'pending_check' | 'checked' | 'pending_sign' | 'signed' | 'completed' | 'rejected'; // Nháp | Đã giao | Đã thực hiện | Trình kiểm tra | Đã kiểm tra | Trình ký | Đã ký | Hoàn thành | Bị trả
...
export const migrateCongVanToLandRecords = async () => {
    // Migrate old 'congvan' records from archive_records to land_records
    try {
        let congVanRecords: ArchiveRecord[] = [];
        if (!isConfigured) {
            const cached = getFromCache<ArchiveRecord[]>(CACHE_KEY_ARCHIVE, []);
            congVanRecords = cached.filter(r => r.type === 'congvan');
            if (congVanRecords.length === 0) return;
        } else {
            ...
            congVanRecords = data as ArchiveRecord[];
        }

        console.log(`Found ${congVanRecords.length} old congvan records to migrate.`);

        const landRecordsToInsert = congVanRecords.map(r => {
            const rData = r.data || {};
            
            // Map status values compatibly
            let status = 'RECEIVED';
            const rStatusStr = r.status as string;
            if (rStatusStr === 'assigned') status = 'ASSIGNED';
            else if (rStatusStr === 'executed') status = 'COMPLETED_WORK';
            else if (rStatusStr === 'pending_check') status = 'PENDING_CHECK';
            else if (rStatusStr === 'checked') status = 'CHECKED';
            else if (rStatusStr === 'pending_sign') status = 'PENDING_SIGN';
            else if (rStatusStr === 'signed') status = 'SIGNED';
            else if (rStatusStr === 'completed') status = 'RETURNED';
            else if (rStatusStr === 'rejected') status = 'REJECTED';
            else if (rStatusStr === 'withdrawn') status = 'WITHDRAWN';

            // Preserve as much information as possible from both old columns and data object
            const safeData: any = {
                id: r.id, // Use original UUID to avoid duplicates
                code: r.so_hieu || rData.so_hieu || rData.code,
                customerName: r.noi_nhan_gui || rData.chu_su_dung || rData.customerName || 'Chưa Cập Nhật',
                receivedDate: r.ngay_thang || rData.ngay_nhan || rData.receivedDate,
                ward: rData.xa_phuong || rData.ward,
                mapSheet: rData.to_ban_do || rData.mapSheet,
                landPlot: rData.thua_dat || rData.landPlot,
                deadline: rData.hen_tra || rData.deadline,
                recordType: '1.2 Công văn',
                status: status,
                content: r.trich_yeu || rData.noi_dung || rData.content,
                assignedTo: rData.assigned_to || rData.assignedTo,
                assignedDate: rData.assigned_date || rData.assignedDate,
                completedWorkDate: rData.ngay_hoan_thanh || rData.completedWorkDate
            };

            const validCols = RECORD_DB_COLUMNS;

            // Retain any additional properties
            for (const key of Object.keys(rData)) {
                if (validCols.includes(key) && safeData[key] === undefined) {
                    safeData[key] = rData[key];
                }
            }
        ...
        if (isConfigured) {
            ...
            const idsToDelete = congVanRecords.map(r => r.id);
            const { error: deleteError } = await supabase
                .from('archive_records')
                .delete()
                .in('id', idsToDelete);
            ...
            // Offline/Local caching migration
            const cachedRecords = getFromCache<any[]>('offline_records', []);
            const updatedRecords = [...cachedRecords];
            
            landRecordsToInsert.forEach(newRec => {
                const idx = updatedRecords.findIndex(r => r.id === newRec.id);
                if (idx !== -1) {
            ...
            saveToCache('offline_records', updatedRecords);

            const cachedArchive = getFromCache<ArchiveRecord[]>(CACHE_KEY_ARCHIVE, []);
            const remainingArchive = cachedArchive.filter(r => !congVanRecords.some(cv => cv.id === r.id));
            saveToCache(CACHE_KEY_ARCHIVE, remainingArchive);
            MOCK_ARCHIVE = remainingArchive;
        }

        console.log('Cong Van migration completed successfully.');
    } catch (error) {
        console.error('Cong Van migration failed:', error);
    }
};
