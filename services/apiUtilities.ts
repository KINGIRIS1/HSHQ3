...
// ============================================================================
// 7. ĐỒNG BỘ THỦ TỤC CỦ VÀ MỚI
// ============================================================================

export interface ProcedureConversion {
    id: string;
    created_at: string;
    thu_tuc_cu: string;
    thu_tuc_moi: string;
...
let MOCK_PROCEDURE_CONVERSIONS: ProcedureConversion[] = [
    { id: 'p1', created_at: new Date().toISOString(), thu_tuc_cu: 'Cung cấp tài liệu đất đai', thu_tuc_moi: '1. Cung cấp dữ liệu đất đai' },
    { id: 'p2', created_at: new Date().toISOString(), thu_tuc_cu: 'Trích lục', thu_tuc_moi: '2.1 Trích lục' },
    { id: 'p3', created_at: new Date().toISOString(), thu_tuc_cu: 'Trích lục Quy hoạch', thu_tuc_moi: '2.2 Trích lục Quy hoạch' },
    { id: 'p4', created_at: new Date().toISOString(), thu_tuc_cu: 'Trích đo', thu_tuc_moi: '2.3 Trích đo' },
    { id: 'p5', created_at: new Date().toISOString(), thu_tuc_cu: 'Cắm mốc', thu_tuc_moi: '2.4 Trích đo cắm mốc' },
    { id: 'p6', created_at: new Date().toISOString(), thu_tuc_cu: 'Tách Thửa', thu_tuc_moi: '2.5 Trích đo Tách - Hợp thửa' },
    { id: 'p7', created_at: new Date().toISOString(), thu_tuc_cu: 'Cung cấp số thửa', thu_tuc_moi: '2.6 Cung cấp số thửa' }
];

export const fetchProcedureConversions = async (): Promise<ProcedureConversion[]> => {
    if (!isConfigured) return MOCK_PROCEDURE_CONVERSIONS;
    try {
        const { data, error } = await supabase
            .from('procedure_conversions')
            .select('*')
        ...
        return data as ProcedureConversion[];
    } catch (error) {
        logError("fetchProcedureConversions", error);
        return MOCK_PROCEDURE_CONVERSIONS;
    }
};

export const saveProcedureConversions = async (records: Partial<ProcedureConversion>[]): Promise<boolean> => {
    if (!isConfigured) {
        const newRecords = records.map(r => ({
            ...r,
            id: generateId(),
            created_at: new Date().toISOString()
        })) as ProcedureConversion[];
        MOCK_PROCEDURE_CONVERSIONS = [...MOCK_PROCEDURE_CONVERSIONS, ...newRecords];
        return true;
    }
    try {
        const newRecords = records.map(r => ({
            thu_tuc_cu: r.thu_tuc_cu?.trim(),
            thu_tuc_moi: r.thu_tuc_moi?.trim()
        }));
        ...
        if (error) {
            ...
            if (error.code === 'PGRST116' || error.code === 'PGRST205' || error.message?.includes('relation "procedure_conversions" does not exist') || error.message?.includes('procedure_conversions') || String(error.code) === '42P01') {
                ...
                MOCK_PROCEDURE_CONVERSIONS = [...MOCK_PROCEDURE_CONVERSIONS, ...newRecordsMock];
                return true;
            }
            throw error;
        }
        ...
        logError("saveProcedureConversions", error);
        return false;
    }
};

export const deleteProcedureConversion = async (id: string): Promise<boolean> => {
    if (!isConfigured) {
    ...
    try {
        ...
        logError("deleteProcedureConversion", error);
        return false;
    }
};

export const deleteAllProcedureConversions = async (): Promise<boolean> => {
    if (!isConfigured) {
    ...
    try {
        ...
        logError("deleteAllProcedureConversions", error);
        return false;
    }
};
